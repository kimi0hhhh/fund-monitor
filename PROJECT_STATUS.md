...
- **Git 拉取远端修复（2026-08-19 20:1x，本机 10719）**：`git fetch` + `git merge --ff-only` 快进到远端 `9d283dd`（本地 `0fe2ab0` 是远端直接祖先，本机无独有提交，纯快进、无冲突）。远端 1 个修复提交：fund_analysis.py `analyze_midterm` 的 `max_tokens` 3500→8000（中长期一次性覆盖 22 只持仓，原 3500 易截断 JSON 致解析失败不落盘）。**未打包（单点小改，按用户偏好攒需求再打包或用户确认后打包）；本机已与远端同频，无需 push**。
  - ⚠️ 本机 `git fetch` 写 `refs/remotes/origin/main` 跟踪分支失败（沙箱 ref 事务不落盘，报 "main new (next fetch will store in remotes/origin)"），但 `.git/FETCH_HEAD` 正常写入；已改用 `git merge --ff-only FETCH_HEAD` 完成合并。下次拉取若遇同样情况照此处理。
...
- **v3.0.0 发版（2026-08-20 00:2x，用户要求版本直接 3.0）**：①**交易成本扣减**：`review_trade_advice` 确定性 pnl 后扣交易成本（新增 `_trade_cost_pct`：申购费 0.1% + 赎回费按持有期 <7天1.5%/<30天0.5%/≥30天0；`_hold_days` 算建议→复盘持有天数），净盈亏判定 result，返回 `pnl_pct`(净)/`gross_pnl`(毛)/`cost_pct`(成本) ②**half-Kelly 仓位**：新增 `_kelly_stats`（从已复盘净盈亏算胜率/平均盈/平均亏）+ `_half_kelly_position`（0.5×(b·p−q)/b，凯利负→0）+ `_apply_half_kelly`（有历史时用 half-Kelly 覆盖 position_change 生成「±X%（half-Kelly）」，冷启动/凯利负保留 LLM 原值），接入 `add_trade_review_from_report`。依据 Novy-Marx & Velikov 2016 RFS（交易成本）+ Kelly 1956（half-Kelly 保守）。隔离单测全绿。**已发版并推 GitHub**：打包 exe 26MB 部署项目根目录（旧版备份 `基金监控_旧版备份.exe`）；git 推 GitHub（github.com 直连先被拦 Empty reply → 确认本地/远端实质差异仅 v3.0.0 改动 → `git push --force-with-lease` 历史线性化，远端 main `5ce2460→c9157ed`）；**tag v3.0.0 已推送**。⚠️ 本次 PAT `ghp_A8GD...TBPU` 已明文使用，提醒到 GitHub 删除/撤销

- **v3.0.1 发版（2026-08-20 00:5x，[object Object] 修复）**：修复加减仓复盘页「📚 复盘经验教训」区块显示 `[object Object]`。根因：v2.0.37 把 `trade_lessons.json` 经验教训从纯文本升级为结构化对象 `{bias_type,pattern,evidence,action}`，前端仍用 `esc(x)` 直接渲染对象 → `[object Object]`（同一处 bug 同时表现为「加减仓复盘页」与「复盘经验教训区块」两块都显示 [object Object]）。修复：新增 `fmtLesson(x)`（兼容旧字符串格式与 v2.0.37+ 结构化对象，渲染为「偏差类型徽章 + 模式 + 案例 + 改进建议」），`refreshTradeReviews` 的 lessons 区块改用 `fmtLesson`。Node 实测结构化 lessons 渲染正常、无 `[object Object]`；`py_compile` 语法检查通过。**已发版并推 GitHub**：打包 exe 部署项目根目录（旧版备份 `基金监控_旧版备份.exe`）；git 提交修复 + 推 main（Git Data API 重放，token 走临时脚本、.git/config 无残留）；**tag v3.0.1 已推送**。⚠️ 本次 PAT `ghp_A8GD...TBPU` 已明文使用，提醒到 GitHub 删除/撤销
